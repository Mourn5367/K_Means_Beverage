...
한국 음료 데이터를 K-means 알고리즘을 사용하여 군집화하고 분석하는 웹 기반 시스템입니다. 사용자는 대화형 인터페이스를 통해 음료 데이터를 탐색하고, 사용자 정의 영양 성분을 입력하여 유사한 음료를 찾을 수 있습니다.
...
- **동적 K 값 설정**: 2~20개 군집 중 원하는 개수 선택 가능
...
### 모델 학습 오류
- 데이터 전처리가 완료되었는지 확인: `data/processed/` 폴더에 CSV 파일 존재 여부
- 원본 데이터 파일 확인: `data/raw/카페인음료.xlsx`, `data/raw/논카페인음료.xlsx`

### JSON 직렬화 오류
- NaN, Inf 값은 자동으로 0.0으로 변환됩니다
- 입력 값은 숫자형이어야 합니다
